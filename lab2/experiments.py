...
class AppHandler(object):
    ...
    def receive_data(self, data, window_size, qd):
        ...
        csv = open("data/queue-" + str(window_size) + ".csv", "a")
        csv.write(str(round(timeReceived, 3)) + "," + str(round(qd, 3)) + "\n")
        csv.close()
...
class Main(object):
    def __init__(self):
        self.directory = 'received'
        self.windowSizes = [1000, 2000, 5000, 10000, 15000, 20000]
        self.filename = None
        self.loss = None
        self.parse_options()
        self.run()
    ...
    def run(self):
        bigCsv = open("data/throughput.csv", "a")
        bigCsv.write("WindowSize,Throughput\n")
        ...
        for window in self.windowSizes:
            ...
            # setup connection
            drop = []
            c1 = TCP(t1, n1.get_address('n2'), 1, n2.get_address('n1'), 1, self.fastRetransmit, a, window=window, drop=drop)
            c2 = TCP(t2, n2.get_address('n1'), 1, n1.get_address('n2'), 1, self.fastRetransmit, a, window=window)

            csv = open("data/queue-" + str(window) + ".csv", "a")
            ...
            finTime = Sim.scheduler.current_time()
            throughput = totsBits / finTime

            print("TotsBits: " + str(totsBits) + " FinTime: " + str(finTime))

            bigCsv.write(str(window) + ","  + str(round(throughput, 3)) + "\n")
